...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const DEFAULT_API_KEY_REF = "KAGI_SEARCH_API_KEY";
     ...
     * card. The API key lives in the credentials service under a fixed reference,
     * so its literal never rides a response. The card learns only whether one is
     * configured and writes it there on save.
     */
    class KagiCardController {
      ...
      async readCredential() {
        let response;
        try {
          response = await this.api.credentials.describe({ refs: [DEFAULT_API_KEY_REF] });
        } catch (_credentialReadFailure) {
          return;
        }
        if (!response.result.ok) return;
        const view = response.result.value.credentials[DEFAULT_API_KEY_REF];
        const next = {
      ...
      refreshCredential(ref) {
        if (ref !== DEFAULT_API_KEY_REF) return;
        this.readCredential();
      }

      async writeKey(value) {
        try {
          await this.api.credentials.set({ ref: DEFAULT_API_KEY_REF, value });
    ...
    const en = {
      ...
      kagiLimitHint: "Fallback result count (1 to 1024) when the tool passes none.",
    ...
    const zh = {
      ...
      kagiLimit: "结果数量",
      kagiLimitHint: "未指定时的默认结果数量（1 到 1024）。",
      kagiSafeSearch: "安全搜索",
